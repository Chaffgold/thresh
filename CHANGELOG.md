...
### Changed
- **Licensing:** the source code is now dual licensed `MIT OR Apache-2.0` (was Apache-2.0 only), applying the license choice listed in the archived, deferred `crates-io-publishing` change. `LICENSE` is renamed `LICENSE-APACHE` and `LICENSE-MIT` is added, both also placed in every publishable crate so that published crates and wheels carry them; the workspace manifest, `thresh-py`'s `pyproject.toml`, the README, `CONTRIBUTING.md` and the `thresh-core::time` rustdoc now agree with `LICENSING.md`, which already said `MIT OR Apache-2.0`.
- **Data and trained-model licensing:** `LICENSING.md`, `TRAINING.md`, the model card and `flight-data-training-pipeline` distinguish OpenSky API terms from dataset-specific grants and require documented rights before distributing OpenSky-derived checkpoints. The API fixture is replaced with a wholly synthetic acquisition sample, the raw London holdout is removed, live captures default to ignored local `data/`, and real-data validation is reopened pending applicable access/use rights. External captures are excluded from git and release artifacts; historical commits and experiment reports remain. Synthetic-truth checkpoints carry the code license; checkpoints trained with ADS-B Exchange data are not released. No trained checkpoint ships today (`test-data/models/` holds random-weight stubs).
...
### Security
- **`thrift` 0.17.0 removed** (GHSA-2f9f-gq7v-9h6m / CVE-2026-43868, excessive memory allocation): `arrow` and `parquet` bumped 53 → 59 in the umbrella `thresh` crate's optional `training-export` feature. Every `parquet` release up to 58.4.0 requires `thrift ^0.17`; 59.0.0 is the first that no longer depends on the `thrift` crate, so a major bump was the only fix. The bump also moves `chrono` 0.4.39 → 0.4.45 (required by `arrow-array` 59), which `thresh-data` picks up via `sgp4`. No source changes were needed, and the Parquet written by 59.3.0 reads back in `pyarrow` identical to the checked-in `imm-samples.parquet` written by 53.4.1. The vulnerable allocation was reachable only by pointing `gen-imm-dataset` / `gen-detector-dataset --trajectories` at a crafted local file.
- **Python training tree** (`python/uv.lock`): `anyio` 4.13.0 → 4.15.1 (GHSA-82r6-8w77-94w6, rated critical, and GHSA-5p39-cfhj-2xmp; transitive via `httpx`) and `setuptools` 81.0.0 → 84.0.0 (GHSA-h35f-9h28-mq5c; transitive via `torch` in the `training` extra). `typing-extensions` 4.15.0 → 4.16.0 follows because `anyio` 4.15.1 requires it.
- **RustSec advisories not flagged by Dependabot**, cleared with semver-compatible `Cargo.lock` updates: `crossbeam-epoch` 0.9.21 (RUSTSEC-2026-0204), `h2` 0.4.19 (RUSTSEC-2026-0258), `rustls` 0.23.45 with `rustls-webpki` 0.103.15 (RUSTSEC-2026-0285), `webbrowser` 1.2.4 (RUSTSEC-2026-0257), `event-listener` 5.4.2 (RUSTSEC-2026-0221, unsound), `wayland-scanner` 0.31.11 (moves its `quick-xml` 0.39.4 → 0.41.0: RUSTSEC-2026-0194, RUSTSEC-2026-0195), and the yanked `der` 0.8.0 and `num-bigint` 0.4.7.
- **Still open** (no compatible fix): `quick-xml` 0.30.0 (RUSTSEC-2026-0194, RUSTSEC-2026-0195) via `zbus_xml` 4.0.0 under `eframe` 0.31's Linux accessibility stack — `thresh-viz --features gui` on Linux only, where it parses XML bundled with the `atspi` crate at compile time; clearing it means moving `egui`/`eframe` off 0.31. Unmaintained, not vulnerable: `paste` (RUSTSEC-2024-0436, via `simba` 0.9 under `nalgebra` 0.33; gone in `nalgebra` 0.35) and `ttf-parser` (RUSTSEC-2026-0192, via `egui`'s `ab_glyph`; no upstream replacement yet).
