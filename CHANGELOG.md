...
- **Licensing of trained models:** `LICENSING.md`, `TRAINING.md`, the model card and `flight-data-training-pipeline` (new design Decision 27, amending 13) no longer say that every trained checkpoint is released under the code license. Terms follow data lineage: synthetic-truth checkpoints carry the code license; checkpoints trained with OpenSky-derived truth are for research and evaluation only; checkpoints trained with ADS-B Exchange data are not released. No trained checkpoint ships today (`test-data/models/` holds random-weight stubs), so nothing already published changes.
