...
- **API terms:** the [OpenSky General Terms of Use & Data License Agreement](https://opensky-network.org/about/terms-of-use)
  (checked 2026-09-20) limits ordinary use in sections 1 and 3 to non-profit
...
- **In this repo:** the checked-in `test-data/training/` samples are synthetic
  ...
  not raw OpenSky records. The acquisition CI fixture,
  `test-data/trajectories/synthetic-sample.parquet`, is also wholly synthetic
  and carries the code license. It replaces the earlier API capture because
  no redistribution permission is known to the maintainer. Historical commits
  still contain that capture; the replacement does not establish permission
  to redistribute it. See `test-data/trajectories/README.md` for provenance.
  The raw London holdout at
  `python/data-acquisition/holdout-london/trajectories.parquet` is also removed
  from the current tree; it remains in git history with the same unresolved
  rights. Reports describing earlier experiments are retained.
- **Real-data access and training:** the OpenSky clients remain available, but
  the next real-data capture/training run is pending confirmation of the
  applicable access and use rights, including any required written permission.
  External captures are excluded from git and release artifacts. Any future
  authorized local capture for training/validation belongs in ignored `data/`,
  with retention governed by the applicable source terms; a local output path
  does not grant permission to acquire, retain, or distribute data.
