## Capability: High-Fidelity Orbital Propagation

### Overview
Provides high-fidelity numerical orbital propagation via nyx-space (preferred, pure Rust) or Orekit (PyO3 bridge fallback) to replace the existing SGP4 propagator for scenarios requiring meter-level accuracy. This capability supports configurable force models including high-order Earth gravity harmonics, atmospheric drag, solar radiation pressure, and third-body perturbations, as well as maneuver modeling and covariance propagation for uncertainty quantification. The nyx-space integration has no Python dependency; the Orekit fallback is feature-gated behind `hifi-orbital`.
...
### Force Model Summary
| Force | Model | Typical Impact (LEO/24h) |
|-------|-------|-------------------------|
| Earth gravity (J2 only) | WGS-84 J2 | ~10 km (already in SGP4) |
| Gravity harmonics (70x70) | EGM96/JGM3 | ~100 m |
| Atmospheric drag | NRLMSISE-00 | ~100 m - 1 km |
| Solar radiation pressure | Cannonball/box-wing | ~10 m |
| Third-body (Sun/Moon) | JPL DE440 | ~10 m |

### Key References
- Montenbruck, O. and Gill, E. "Satellite Orbits," Springer, 2000
