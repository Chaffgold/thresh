## Capability: RCS Computation via PyO3 Bridge

### Overview
Provides a PyO3-based bridge to PyPOFacets or Open RCS for offline computation of monostatic radar cross-section (RCS) from 3D target geometry. This capability enables users to import STL or faceted models, compute RCS as a function of aspect angle at a specified frequency using physical optics approximation, and export the resulting RCS table as JSON for runtime use by the Swerling RCS and radar equation modules. The entire capability is feature-gated behind `rcs-compute` so that Python and the RCS solver are not required for normal operation.
...
The PO approximation computes RCS by:
1. For each facet, determine if it is illuminated (n_hat . k_inc < 0)
2. Compute the induced surface current J = 2(n_hat x H_inc) on illuminated facets
3. Integrate the far-field scattered electric field contribution from each facet
4. Sum coherently over all illuminated facets to get total scattered field
5. RCS = (4*pi*R^2 * |E_s|^2) / |E_i|^2

### Key References
- Rius, J.M. et al. "High-Frequency RCS of Complex Radar Targets in Real-Time," IEEE Transactions on Antennas and Propagation, 1993
- PyPOFacets: Python Physical Optics Facets code for RCS computation
- Knott, E.F. "Radar Cross Section Measurements," SciTech Publishing, 2004
