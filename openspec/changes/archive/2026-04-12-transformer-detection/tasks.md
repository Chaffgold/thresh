...
- [x] 4.3 Create a tiny synthetic ONNX test model (< 1 MB, random weights, correct RT-DETR input/output shapes) under `test-data/models/` — generated by `scripts/generate_test_model.py` (2.4 KB, input `point_cloud` 1x1000x4, outputs `boxes` 1x100x7 + `scores` 1x100x1)
...
- [x] 7.2 CI job: `onnx-tests` in `.github/workflows/ci.yml` runs `cargo test -p thresh-inference --features onnx` on CPU — gated on `schedule` (weekly) and `workflow_dispatch` since ORT binary download can be slow
