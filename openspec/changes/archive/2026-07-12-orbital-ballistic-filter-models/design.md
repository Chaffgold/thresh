...
- **Orbital head birth velocity:** a single radar position fixes 3 of 6 states; the current `birth_track` zero-velocity init is hopeless for a 7.5 km/s LEO target. Options: two-point differencing at confirmation, or circular-orbit velocity prior from the position. Decide during implementation of Decision 7's benchmark path; the head's velocity prior width depends on the answer.
  - **RESOLVED (task 5.5): circular-orbit velocity prior.** The birth mean keeps zero velocity; the prior is sized so that width is honest about the unknown circular-speed direction: per-axis variance `v_c²/3 = μ/(3‖r‖)` (`v_c = √(μ/r)` ≈ 7.6 km/s at LEO). `TrackHead::orbital()` carries the static LEO value (2.0e7 m²/s²) and `birth_track` (`birth_covariance` in `tracker.rs`) refines it from the measured radius whenever the detection is a plausible ECI position (`‖r‖ >` equatorial radius) — a GEO-radius birth therefore gets a correspondingly tighter prior. Two-point differencing at confirmation was rejected as new machinery for no additional information: with the circular prior, the EKF's first re-association already performs implicit two-point differencing — after one `F·P·Fᵀ` propagation the position–velocity cross-covariance `dt·σ_v²` dominates the position variance `dt²·σ_v²`, so the velocity Kalman gain approaches `1/dt` and the update assigns `v ≈ Δz/Δt` — whereas explicit differencing would need deferred birth-state plumbing through the association layer. Regression-tested by `orbital_track_follows_orbit_across_measurement_gap` (velocity converges from position scans, then a 90 s gap prediction stays on-orbit while straight-line extrapolation diverges by tens of km) and `orbital_birth_refines_velocity_prior_from_measured_radius`. If Decision 7's benchmark calibration shows birth-velocity-limited MOTA, explicit differencing can be layered on then.
- **Ballistic scenario visibility window:** whether the MRBM radar sees upper midcourse (β unobservable, tests covariance honesty) or only reentry (β observable throughout) — pick after first truth-generation runs show realistic pass geometry.
  - **RESOLVED (task 6.3): midcourse + reentry, one continuous window.** The first truth runs of the `ballistic-mrbm` profile (equatorial due-north launch, 52 m/s² thrust for 65 s, 38° pitch kick, β = 2000 kg/m²) measured apogee 231 km, flight time 527 s, ground range ≈ 1073 km, impact 72 km from the downrange station at 9° N. From that station the target rises above the horizon at t = 69 s — just after burnout, at ~80 km altitude and ~1000 km ground distance — and stays continuously visible through reentry and impact, so the pass is [69, 527] s. The scenario therefore exercises **both** regimes in one pass: the β-unobservable exo-atmospheric stretch (where the β random walk must keep re-inflating the covariance) and the β-observable high-drag reentry, which is strictly more informative than a reentry-only geometry and required no station-placement contortions. Note the task's "~300 s flight" sketch was physically inconsistent with the "~1000 km range" requirement: a 1000 km trajectory with a real exo-atmospheric midcourse flies ~530 s, and forcing 300 s would need a fully depressed shot (apogee < 90 km) that never leaves the atmosphere, defeating the phase coverage the benchmark exists to gate. The range constraint was kept; flight time follows from the physics (same record in `ballistic-mrbm.toml`'s comments).
