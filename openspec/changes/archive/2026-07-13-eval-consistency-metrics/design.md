...
- **Two-point-difference birth covariance honesty** — track birth seeds P from head config, not from measurement geometry; early-life NEES samples may run hot for reasons unrelated to Q. Decide during calibration whether ANEES should skip the first k post-confirmation samples per track (a `warmup` knob on `sequence_anees`) or whether confirmed-only collection already suffices.
  - **RESOLVED (task 6.6 calibration): split verdict.** *ANEES needs no warmup knob* — confirmed-only collection (3-of-5 confirmation) already skips the transient: the honest seeded `synth-cv-clean` run measured ANEES 3.2636 over 145 samples against a theoretical [2.6145, 3.4116], inside without any skipping, so `sequence_anees` stays knob-free. *ANIS does need one*: the tracker accumulator counts every hard-assigned KF update, including each track's first updates, whose innovations carry the unconverged birth velocity (zero-velocity birth + σ_v ≈ 32 m/s prior vs 200–280 m/s targets). Measured transient on the seeded scenario: warmup 0 → ANIS 3.71 (above the [2.62, 3.42] interval), 1 → 3.59 (still above), 2 → 3.28 (inside), 3 → 2.87, 4 → 2.58 (below the lower bound — steady state runs mildly conservative because the tracker carries Q = 10 against zero-process-noise truth). `NIS_WARMUP_UPDATES = 2` in the tracker is the minimal window whose statistic sits inside the interval — least masking for a dishonesty gate; the constant's doc records the sweep.
...
## Implementation-Time Divergences (task 7.3)

- **`synth-cv-clean` was not the textbook case as committed — fixed at the generator, not papered over with wide bounds.** Decision 5 premised the demonstration on "the CV tracker on CV truth with known noise… genuinely should sit inside the 95% interval", but the synthetic runner produced RAE radar noise whose Cartesian cross-range error grows with range (σ_az = σ/10 000 ⇒ 57–185 m across the scenario's 11–37 km targets) while the tracker's R is isotropic 50 m: measured ANEES 13.08 against [2.60, 3.42] — the configuration itself was the covariance lie. Fix: an explicit `measurement_model = "cartesian"` scenario parameter (isotropic Gaussian of `measurement_noise_sigma` drawn directly on truth positions, exactly matching R); the default remains the RAE model for every other scenario. With it, honest ANEES 3.2636 / ANIS 3.2760 sit inside their intervals. The RAE-vs-isotropic-R overconfidence is documented in the TOML and is a real finding for the radar-based scenarios (converted-measurement R is future work).
- **`run_scenario` drew from an unseeded thread-local RNG**, so no synthetic scenario was reproducible (the spec scenario "Deterministic gate outcome" was unsatisfiable). Added `run_scenario_with_rng` in `thresh-synth` and switched the benchmark runner to a fixed-seed `StdRng`, matching the ballistic/orbital/ADS-B runners. Bitwise determinism across runs is now tested end-to-end.
- **The deliberate-regression knob is R, not Q.** Task 6.6's sketch scales process noise, but cv-clean truth is exact CV with zero process noise — scaling the filter's (already negligible) Q cannot manufacture overconfidence there. The equivalent covariance lie is an understated/overstated tracker R: the test-only `tracker_noise_sigma` parameter decouples the tracker's R from the generator's noise. Measured: σ = 5 (10× understated) → ANEES 254.07 / ANIS 123.93, both far above the upper bounds while MOTA 0.76 still clears its 0.5 floor; σ = 150 (3× overstated) → ANEES 1.09 / ANIS 1.50 below the lower bounds with MOTA 0.65 passing. (σ = 500 was rejected as the underconfident probe: the over-smoothed estimates lag outside the MOTA match threshold, so the lie stops being invisible to MOT metrics.)
- **GOSPA cost matrix is clamped `min(d, c)^p`, not raw `d^p`** (task 4.1's literal wording): the Hungarian solver optimizes the raw entries and only gates after optimization, so an unclamped entry far above `c^p` can steer the matching to a gated-suboptimal solution, violating the spec's optimality requirement. The `clamped_cost_yields_optimal_assignment` test pins a concrete counter-example; the module docs carry the constant-offset equivalence proof.
- **Smaller deltas:** `UpdateOutcome` derives `Debug + Clone` (needed for recorded outcome streams in tests); `thresh-eval` gained the workspace-pinned `nalgebra` dependency (the spec'd `DVector`/`DMatrix` API is unimplementable without it); accumulator `mean`/`bounds`/`verdict` return `Option`, `None` only when empty (keeps the never-NaN contract at n = 0); the per-track helper is a thin `PerTrackConsistency` struct enforcing sorted-ID iteration; `build_benchmark_result` also takes the tracker (ANIS is read inside the one shared path) and wires **five** runners, not four — the ballistic runner landed after tasks.md was written.
