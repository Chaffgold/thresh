...
- [ ] 1.1 Read existing `crates/thresh-association/src/hungarian.rs` and identify distinct phases (row/column reduction, augmenting-path search, label/slack update, assignment extraction)
- [ ] 1.2 Extract row/column reduction into `reduce_cost_matrix()` with its own unit tests
- [ ] 1.3 Extract augmenting-path search into `find_augmenting_path()` with its own unit tests
