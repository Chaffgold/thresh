...
- The deferred `crates-io-publishing` plan has been restored to the active
  backlog with all 26 tasks open; it has not authorized or started publishing.
...
**Decision:** Track B implements one learned model first: an IMM mode-probability classifier that ingests a **filter-state** history (state vector plus a projection of the covariance, produced by running the classical tracker over simulated measurements) and outputs `(p_CV, p_CA, p_CTRV, p_coord_turn)`. Its ONNX checkpoint is loaded by a new `learned-imm` feature gate in `thresh-filter` that replaces only the posterior mode-probability estimate after the analytic measurement update. The analytic mode-transition matrix and interaction-step mixing remain unchanged, as specified in Decision 19.
...
If the third bullet fails, retain the random stub and do not commit or publish the trained checkpoint. Document the regression and continue evaluation; an experimental label or disabled-by-default feature does not waive this gate (Decision 29). Distribution also requires the documented rights in Decision 27.
...
### 29. Strict no-regression release gate (amends Decision 12)

**Decision (2026-09-20, maintainer confirmed):** All three Track B acceptance
criteria must pass before committing or publishing a trained classifier:
held-out accuracy of at least 0.70, passing IMM tests, and downstream tracker
MOTA no worse than the analytic baseline. Documented training and distribution
rights remain an additional requirement under Decision 27.

**Rationale:** The original experimental-release exception contradicted the
all-criteria gate in the capability spec and task 6.8. The maintainer selected
the strict gate. A regressing model may be investigated in authorized local
