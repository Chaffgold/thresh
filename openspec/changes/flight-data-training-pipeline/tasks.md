...
- [ ] 6.8 Track B exit criterion: on a held-out trajectory split, classifier accuracy ≥ 0.70 against analytic labels AND existing IMM tests still pass AND downstream tracker MOTA on the synthetic ADS-B scenario is no worse than with analytic mode probabilities. _Deferred (mirrors the Track A detector's 7.9): the full pipeline (train → export → ONNX → Rust adapter) is implemented and verified end-to-end on synthetic data, and the IMM suite passes with the feature on. The ≥0.70 held-out-accuracy + MOTA-no-worse verification requires a real training run (training is a non-CI goal per Decision 7); until then the checked-in `imm_mode_classifier.onnx` is a random-weight **stub** and `learned-imm` stays off by default._
