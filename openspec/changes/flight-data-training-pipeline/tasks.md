...
- [x] 4.1 Add a new `thresh_synth::radar::from_trajectory(trajectory, sensor_pose, frame=ENU)` API that consumes a canonical trajectory (read via the Rust-side Parquet reader or passed in directly as a slice of states) and emits paired `(point_cloud, gt_boxes_3D)` snapshots at the configured sensor sample rate. _Implemented as `thresh_synth::radar_pcl::from_trajectory(&[(Vec<Waypoint>, TargetProfile)], &RadarPclConfig, rng)` — takes target waypoint slices directly (Parquet ingestion is the Phase 2/3 acquisition layer, still blocked on network); sensor-centred ENU per Decision 10._
- [x] 4.2 Add a parallel `thresh_synth::radar::measurements_from_trajectory(...)` API that emits measurement-level outputs (range / azimuth / elevation / Doppler with noise + miss probability per existing synth config) suitable as input to the classical tracker. _Implemented as `radar_pcl::measurements_from_trajectory(waypoints, rcs_m2, &RadarConfig, doppler_sigma, rng)`; reuses `generate_radar_with_rcs` for range/az/el + Pd-miss and augments with a noisy radial range-rate (Doppler)._
- [x] 4.3 Reuse existing RCS, clutter, and detection-probability models; the only new piece is the trajectory ingestion. _Reuses `measurement_gen::{compute_snr, albersheim_pd, generate_radar_with_rcs, RadarEquationConfig, RadarConfig}` — no radar-equation logic duplicated._
- [x] 4.4 Snapshot output (Track A path) should match the ONNX input shape exactly: 1000 points × `[x, y, z, intensity]` plus a variable number of ground-truth boxes (padded to 100, with a validity mask) per `[x, y, z, L, W, H, yaw]`. _`PointCloudSnapshot`: `points` = exactly `NUM_POINTS=1000` × `[x,y,z,intensity]`; `boxes`/`box_validity`/`box_classes` = exactly `MAX_BOXES=100`, boxes `[x,y,z,L,W,H,yaw]` zero-padded with a `bool` validity mask + class index._
- [x] 4.5 Unit test: feed in a straight-line constant-velocity trajectory, assert (a) the synthesised PCL has a cluster of returns near the predicted target position and the GT box matches the trajectory state, and (b) the measurement stream has reasonable noise characteristics (residuals consistent with the configured sensor model). _5 tests: `cv_target_produces_cluster_near_gt_box`, `measurements_have_expected_noise`, `snapshot_shapes_match_onnx_contract`, `target_beyond_max_range_has_no_valid_box`, `detection_class_indices`._
- [ ] 4.6 Add a Python binding (via `pyo3` or just CSV/parquet I/O) so the training scripts can call both APIs from PyTorch. _Deferred: needs a `pyo3`/Parquet feature surface; the pure-Rust core (4.1–4.5) lands first._
- [ ] 4.7 Spike check: visualise a generated PCL batch and a measurement-stream trace, verify both "look reasonable" before committing to training in Phases 5 and 7. _Deferred: requires manual visual inspection / plotting, not feasible headless in the implementation sandbox._
