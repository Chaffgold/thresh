...
- [x] 4.1 Added `OnnxDetector::reload_with_weights()` as a documented stub behind `#[cfg(feature = "onnx")]`. ORT does not support runtime weight injection; the method returns an error directing users to `NativeDetector` with SafeTensors instead.
- [x] 4.2 Atomic weight swap documented as infeasible for ORT sessions. The `reload_with_weights` stub explains the required approach (protobuf manipulation + session re-creation). Real weight override is supported via `NativeDetector`.
...
- [x] 5.6 Added `test_safetensors_weight_loading_for_detector_integration` (non-feature-gated) exercising the `SafeTensorsLoader` + `WeightSet` API for detector-style access. Also added `test_onnx_reload_with_weights_returns_stub_error` behind `#[cfg(feature = "onnx")]` + `#[ignore]`.
