# astro-time Specification

## Purpose
Time-scale-aware absolute epoch type in `thresh-core` carrying its scale (UTC, TAI, GPS, TT) explicitly, with leap-second-correct arithmetic, unambiguous serde serialization, and interoperability with the tracker lane's relative-seconds convention.
