# automotive-class-taxonomy Specification

## Purpose
Define the native automotive object classes in `TargetClass` (Car, Truck,
Bus, Motorcycle, Bicycle, Pedestrian) and the lossless nuScenes category
mapping (`thresh-data::category::map_category`): no automotive category may
resolve to an aerospace class, with `Unknown` reserved for genuinely
