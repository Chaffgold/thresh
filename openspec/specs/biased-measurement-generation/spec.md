# biased-measurement-generation Specification

## Purpose
Opt-in application of the tropospheric-refraction and ionospheric-group-delay biases to synthetic radar measurement generation — bias applied to the true RAE geometry before measurement noise (bias-then-noise), defaulting to off with byte-for-byte unchanged legacy output and bitwise-invariant calibrated benchmarks, plus an end-to-end demonstration that the bias class degrades an uncorrected tracker measurably and is recovered by applying the atmospheric corrections at the RAE→Cartesian conversion seam.
