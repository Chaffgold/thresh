# consistency-benchmark-gates Specification

## Purpose
Consistency gating in the `thresh-data` benchmark harness: scenario TOMLs may declare optional two-sided ANEES / average-NIS intervals, benchmark results carry the corresponding statistics (or mark them explicitly absent), and the regression check enforces the bounds so miscalibrated filter covariances fail the gate even when MOT baselines still pass.
