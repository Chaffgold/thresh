# filter-consistency-metrics Specification

## Purpose
NEES/NIS filter-consistency evaluation in `thresh-eval`: per-step and time-averaged statistics with two-sided chi-squared bounds and three-way verdicts, validated on a linear-Gaussian reference problem so miscalibrated filter covariances are detectable even when point estimates look fine.
