...
## Purpose
Aircraft and missile trajectories generated by JSBSim 6DOF flight dynamics models, gated behind the `jsbsim` Cargo feature so that builds without it need no Python, PyO3, or JSBSim. Covers model initialization from initial conditions, waypoint and autopilot-command trajectory definitions, state-history extraction at a user-specified sample rate, maneuver limits enforced by the flight dynamics model, and conversion to the thresh trajectory format used by measurement generators and tracker evaluation pipelines.
