# tropospheric-refraction Specification

## Purpose
Deterministic tropospheric refraction modeling for ground-station radar measurements — apparent-elevation bending and excess range from a Bean–Dutton exponential refractivity profile integrated along the spherically-stratified ray path, a classic 4/3-Earth effective-radius cheap tier cross-checked against it, an explicit minimum-elevation validity band, and an inverse-model correction with measured residuals under parameter mismatch, applicable before RAE→Cartesian conversion.
